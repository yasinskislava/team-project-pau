...
export default function headerFunc() {
    ...
    pages[1].addEventListener("click", () => {
        ...
        document.querySelector(".favorites").style.left = "20px";
    ...
    pages[0].addEventListener("click", () => {
        ...
        document.querySelector(".home").style.left = "20px";
    });
    const trigger = document.querySelector("header > button");
    const menu = document.querySelector(".menu");
    trigger.addEventListener("click", (e) => {
        e.preventDefault();
        document.querySelector(".title").style.color = `${coloring(document.querySelector(".title").style.color)}`;
        menu.style.transform = `translateY(${scaling(menu.style.transform.slice(-4, -3))})`;
